...
{
    // احذف validation للـ image و Category
    ...
    if (ModelState.IsValid)
    {
        ...
        if (product != null)
        {
            ...
            if (image != null && image.Length > 0)
            {
                ...
                if (!string.IsNullOrEmpty(product.image))
                {
                    var oldImagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images", product.image);
            ...
            // إذا ما في صورة جديدة، خلي الصورة القديمة زي ما هي (ما تعملش شي)
